{-# LANGUAGE BangPatterns, DeriveAnyClass, DeriveGeneric, NamedFieldPuns #-}
{-# LANGUAGE ParallelListComp, RecordWildCards, ViewPatterns             #-}
...
boolOp :: (Eq c, Ord c, Hashable s, Hashable t, Ord s, Ord t)
       => (Bool -> Bool -> Bool) -> DFA s c -> DFA t c -> DFA (Trapped s, Trapped t) c
boolOp op d1'0 d2'0 =
  let inputs = nub $ letters d1'0 ++ letters d2'0
      ...
      fs = fromList [(s, t) | (s, t) <- ss
                               , isFinal d1 s `op` isFinal d2 t]
  in DFA{ initial = (initial d1, initial d2)
        , transition = trans
        , final = fs
        }
...
determinize n@NFA{..} =
  let sts  = fromList $ map nubbing $ filterM (const [True, False]) $ HS.toList $ statesNFA n
      ...
      transition = fromList [((ss, l), nubbing $ concat $ mapMaybe (\s -> HS.toList <$> M.lookup (s, l) transitionNFA) $
                                         ss)
                            | ss <- HS.toList sts
                            , l <- inputs
                            ]
...
expandLetters cs d0 =
  let DFA{transition = trans, ..} = changeState Normal d0
      ...
      transition = trans `M.union` fromList [((Normal q, c), Trap)
                                            | c <- Set.toList ls
